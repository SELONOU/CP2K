...
def clean_velocity_xyz(infile):
    ...
    while i < nlines:
       ...
       # break
        ...
        if i < nlines:
            # skip your custom comment line, replace with simple one
...
time_ps_all = log_data[:, 1] / 1000.0  # convert fs → ps
...
dt_fs = (time_ps[1] - time_ps[0]) * 1000.0  # dt in fs

print(f"Time window: {start_time_ps} → {end_time_ps or time_ps[-1]:.3f} ps | dt = {dt_fs:.3f} fs | Steps = {nsteps} | T = {temperature:.1f} K")

# --- Clean velocity XYZ and read with ASE ---
clean_xyz_file = clean_velocity_xyz(traj_file)

images = read(clean_xyz_file, index=":")
if len(images) != len(time_ps_all):
    print(f"Warning: trajectory frames ({len(images)}) and log lines ({len(time_ps_all)}) count differ.")

if end_idx > len(images) or start_idx > len(images):
    print(f"Error: Selected frame indices [{start_idx}:{end_idx}] out of range for trajectory with {len(images)} frames.")
...
# --- Manually parse velocity-only XYZ file ---
velocities = []
...
velocities = np.array(velocities)  # shape: (n_frames, n_atoms, 3)
print("Parsed velocities shape:", velocities.shape)
...
print("Example velocity values (frame 0):")
for i, vel in enumerate(velocities[0]):
    print(f"Atom {i}: {vel}")
if velocities.shape != (nsteps, n_atoms, 3):
    print(f"Error: Unexpected velocities array shape: {velocities.shape}")
    os.unlink(clean_xyz_file)
    sys.exit(1)
...
if mass_weighted:
    ...
    print("Atomic masses used for weighting:", masses)
    if atom_indices is not None:
    ...
    if (masses <= 0).any():
        ...
        print("Weights (sqrt masses):", weights)
        # safer element-wise multiplication
        for iatom in range(len(weights)):
            velocities[:, iatom, :] *= weights[iatom]
        print("Applied mass weighting successfully")
...
# --- Cleanup temporary cleaned xyz file ---
os.unlink(clean_xyz_file)
